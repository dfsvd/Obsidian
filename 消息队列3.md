...
`[此处插入图片：消息队列运作原理图，展示多任务读写及阻塞唤醒流程]`
...
`[此处插入图片：普通入队 vs 紧急入队 示意图]`

### 三、 常用函数接口详解

FreeRTOS 提供了丰富的队列操作 API，以下为最常用的 10 个函数详解。

#### 1. 动态创建消息队列：`xQueueCreate`
- **函数原型**：
  ```c
...
- **参数详解**：
    - `uxQueueLength`：队列深度，即队列能够存储的最大消息单元数目。
    - `uxItemSize`：每个消息单元的大小（字节）。**务必准确**，否则数据存取会越界或截断。
- **返回值**：
    - **成功**：返回一个有效队列句柄。
    - **失败**：返回 `NULL`（通常因堆内存不足，RAM 无法分配成功）。

#### 2. 静态创建消息队列：`xQueueCreateStatic`
- **函数原型**：
  ```c
...
- **函数原型**：
  ...
  ```
- **功能描述**：向队列**尾部**发送一条消息。等同于 `xQueueSendToBack`。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvItemToQueue`：指针，指向要发送的数据。系统会将该指针指向的内容拷贝 `uxItemSize` 字节到队列中。
    - `xTicksToWait`：阻塞超时时间（Tick）。
        - 若队列满且此值为 `0`，函数立即返回。
        - 若设置为 `portMAX_DELAY` 且宏 `INCLUDE_vTaskSuspend` 为 1，任务将无限期挂起（死等）直到队列有空位。
- **返回值**：
    - `pdTRUE`：消息发送成功。
    - `errQUEUE_FULL`：队列已满，消息发送失败。

#### 4. 中断中队尾发送：`xQueueSendFromISR`
- **函数原型**：
  ```c
  ...
  ```
- **功能描述**：在中断服务程序（ISR）中向队列**尾部**发送消息。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvItemToQueue`：指向要发送数据的指针。
    - `pxHigherPriorityTaskWoken`：**出参**。如果入队操作导致一个优先级高于当前被中断任务的任务解锁（变为就绪态），则该参数会被置为 `pdTRUE`。从 FreeRTOS V7.3.0 起，此参数可选，可设置为 `NULL`。
- **返回值**：
    - `pdTRUE`：消息发送成功。
    - `errQUEUE_FULL`：队列已满，消息发送失败。
- **注意事项**：若 `pxHigherPriorityTaskWoken` 变为 `pdTRUE`，应在 ISR 退出前调用 `portYIELD_FROM_ISR()` 进行上下文切换。

#### 5. 队首发送消息（紧急）：`xQueueSendToFront`
- **函数原型**：
  ```c
  ...
  ```
- **功能描述**：向队列**头部**发送一条消息，实现 LIFO 或紧急插入。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvItemToQueue`：指向要发送数据的指针。
...
#### 6. 中断中队首发送：`xQueueSendToFrontFromISR`
- **函数原型**：
  ```c
  ...
  ```
- **功能描述**：在中断服务程序（ISR）中向队列**头部**发送消息。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvItemToQueue`：指向要发送数据的指针。
    - `pxHigherPriorityTaskWoken`：**出参**（同 `xQueueSendFromISR`）。
- **返回值**：
    - `pdTRUE`：消息发送成功。
    - `errQUEUE_FULL`：队列已满，消息发送失败。

#### 7. 接收并删除消息：`xQueueReceive`
- **函数原型**：
  ```c
  ...
  ```
- **功能描述**：从队列头部读取一条消息，并将其**从队列中删除**。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvBuffer`：指针，指向用于保存接收数据的缓冲区。必须确保该缓冲区大小至少等于队列创建时的 `uxItemSize`。
    - `xTicksToWait`：阻塞超时时间（Tick）。
        - 若队列空且此值为 `0`，函数立即返回。
        - 若设置为 `portMAX_DELAY` 且宏 `INCLUDE_vTaskSuspend` 为 1，任务将无限期挂起（死等）直到队列有数据。
- **返回值**：
    - `pdTRUE`：成功接收到消息。
    - `pdFALSE`：队列为空，未接收到消息。

#### 8. 窥视消息（不删除）：`xQueuePeek`
- **函数原型**：
  ```c
...
#### 9. 中断中接收消息：`xQueueReceiveFromISR`
- **函数原型**：
  ```c
  ...
  ```
- **功能描述**：在中断服务程序（ISR）中从队列读取并删除消息。
- **参数详解**：
    - `xQueue`：目标队列的句柄。
    - `pvBuffer`：指向保存数据的缓冲区。
    - `pxHigherPriorityTaskWoken`：**出参**。在使用前必须初始化为 `pdFALSE`。如果接收操作导致一个高优先级任务解锁（例如因为队列有了空位而解除写入阻塞），该参数会被置为 `pdTRUE`。
- **返回值**：
    - `pdTRUE`：成功接收到消息。
    - `pdFALSE`：队列为空。

#### 10. 毫秒转节拍宏：`pdMS_TO_TICKS`
- **函数原型**：
  ```c
  TickType_t pdMS_TO_TICKS( uint32_t xTimeInMs );
  ```
- **功能描述**：辅助宏，用于将毫秒（ms）时间转换为系统节拍数（Ticks）。仅在 FreeRTOS V8.1.0 及以上版本可用。
- **参数详解**：
    - `xTimeInMs`：时间，单位为毫秒。
- **返回值**：
    - 对应的系统节拍数 `TickType_t`。
...
1.  **先创建后使用**：务必在使用任何队列操作函数前检查队列是否创建成功（句柄不为 `NULL`）。
2.  **缓冲区大小匹配**：`uxItemSize` 必须与实际数据结构大小严格一致。接收端的 `pvBuffer` 空间也不得小于此大小，否则会发生内存踩踏。
